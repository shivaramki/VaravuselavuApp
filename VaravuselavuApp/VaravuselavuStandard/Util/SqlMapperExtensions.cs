...
namespace Dapper.Contrib.Extensions
{
    public static partial class SqlMapperExtensions
    {
        ...
        private static readonly ConcurrentDictionary<RuntimeTypeHandle, IDictionary<string, string>> QuotedColumnMap = new ConcurrentDictionary<RuntimeTypeHandle, IDictionary<string, string>>();
        ...
        private static IDictionary<string, string> QuotedColumnCache(Type type)
        {
            IDictionary<string, string> map;
            if (QuotedColumnMap.TryGetValue(type.TypeHandle, out map))
                return map;

            var quotedColumnMap = new Dictionary<string, string>();
            var quotedColumns = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is Quote)).ToList();

            foreach (var quotedColumn in quotedColumns)
            {
                var col = (Quote)quotedColumn.GetCustomAttributes(typeof(Quote), false).FirstOrDefault();
                quotedColumnMap.Add(quotedColumn.Name, col.ColumnName);
            }

            QuotedColumnMap[type.TypeHandle] = quotedColumnMap;
            return quotedColumnMap;
        }
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (VaravuselavuStandard.Util.TypeExtensions.IsInterface(type))
            {
        ...
        public static IEnumerable<T> GetAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!VaravuselavuStandard.Util.TypeExtensions.IsInterface(type)) return connection.Query<T>(sql, null, transaction, commandTimeout: commandTimeout);
        ...
        private static string GetTableName(Type type)
        {
            ...
            else
            {
                ...
                {
                    name = type.Name + "s";
                    if (VaravuselavuStandard.Util.TypeExtensions.IsInterface(type) && name.StartsWith("I"))
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var quotedColumns = QuotedColumnCache(type);
            ...
            for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count; i++)
            {
                ...
                if (quotedColumns.TryGetValue(property.Name, out columnName))
                    adapter.AppendQuotedColumnName(sbColumnList, columnName);
                else
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < nonIdProps.Count; i++)
            {
                ...
                var columnName = property.Name;
                if (quotedColumnMap.TryGetValue(property.Name, out columnName))
                    adapter.AppendQuotedColumnNameEqualsValue(sb, columnName);  //fix for issue #336
                else
    ...
    [AttributeUsage(AttributeTargets.Property)]
    public class Quote : Attribute
    {
        public Quote(string columnName)
        {
...
public partial interface ISqlAdapter
{
    ...
    void AppendQuotedColumnName(StringBuilder sb, string columnName);
    void AppendColumnNameEqualsValue(StringBuilder sb, string columnName);
    void AppendQuotedColumnNameEqualsValue(StringBuilder sb, string columnName);
}
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    public void AppendQuotedColumnName(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("[{0}]", columnName);
    }

    public void AppendQuotedColumnNameEqualsValue(StringBuilder sb, string columnName)
    {
        sb.AppendFormat("[{0}] = @{1}", columnName, columnName);
    }
}
